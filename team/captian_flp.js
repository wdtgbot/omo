...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            authorCodeList = ['df3fef08269f41e3a022c4dea3e3c15e']
            $.bean = 0;
            ...
            $.activityUrl = `https://lzkj-isv.isvjcloud.com/pool/captain/${$.authorNum}?activityId=${$.activityId}&signUuid=${encodeURIComponent($.authorCode)}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
...
async function captain() {
    ...
    if ($.token) {
        await getMyPing();
        if ($.secretPin) {
            await task('common/accessLogWithAD', `venderId=${$.activityShopId}&code=46&pin=${encodeURIComponent($.secretPin)}&activityId=${$.activityId}&pageUrl=${$.activityUrl}&subType=&adSource=`, 1);
            ...
            if ($.activityContent) {
                ...
                if (taskList.length) {
                    times = taskList.length > 4 ? 4 : taskList.length
                            ...
                            switch (function_id) {
                                case 'saveCaptain':
                                    if (data.data.signUuid) {
                                        $.log("创建队伍成功")
                                        ownCode['signUuid'] = data.data.signUuid
                                    }
                                    break;
...
function bindWithVender(venderId, shopId, channel) {
    let opt = {
        url: `https://api.m.jd.com/client.action?appid=jd_shop_member&functionId=bindWithVender&body=${encodeURIComponent(JSON.stringify({ "venderId": venderId, "shopId": shopId, "bindByVerifyCodeFlag": 1, "registerExtend": {}, "writeChildFlag": 0, "channel": channel },))}&client=H5&clientVersion=9.2.0&uuid=88888`,
        headers: {
            Host: 'api.m.jd.com',
            Accept: '*/*',
            ...
            Referer: `https://shopmember.m.jd.com/shopcard/?venderId=${venderId}}&channel=${channel}&returnUrl=${encodeURIComponent($.activityUrl)}`,
